...
//
// Зачем нужен: чтобы подключить мост Tuya <-> SmartThings, НЕ трогая
// server.js (2100+ строк рабочей логики). Здесь мы:
...
function mountBridge() {
    ...
    // Отдаём index.html с автоматически вставленными скриптами моста
    app.use(injectClientScript({ rootDir: __dirname }));

    // REST API + WebSocket + планировщик
    ...
    console.log('[bridge] Мост Tuya <-> SmartThings подключён поверх server.js');
}

// server.js может инициализироваться асинхронно — даём событийному циклу
// завершить текущий тик перед монтированием маршрутов.
setImmediate(() => {
    try {
        mountBridge();
    } catch (error) {
