...
// Секреты и настройки моста: .env (dotenv) + fallback + переопределение из админки.
// ==========================================
...
function ensureLoaded() {
    ...
    try {
        if (fs.existsSync(BRIDGE_SETTINGS_FILE)) {
            overrides = sanitize(fs.readJsonSync(BRIDGE_SETTINGS_FILE) || {});
        }
    } catch (error) {
        console.error('[bridge/config] bridge-settings.json:', error.message);
        overrides = {};
    }
    ...
    try {
        ...
        console.error('[bridge/config] settings.json:', error.message);
    }
}
