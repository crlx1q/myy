...
function cacheToken(result) {
    ...
    tokenCache = {
        ...
        // обновляем за 60 секунд до фактического истечения
...
async function request(method, pathWithQuery, body = null, { retryOnAuthError = true } = {}) {
    ...
    // 1010/1011/1012 — токен невалиден/просрочен: один повтор с новым токеном
...
// Health check: получаем токен и (опционально) инфо по устройству
