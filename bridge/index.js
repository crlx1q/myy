...
function buildRouter() {
    ...
    router.get('/api/bridge/history', (req, res) => {
        const limit = Math.min(1000, Number(req.query.limit) || 200);
        res.json({ success: true, history: engine.readHistory(limit) });
    });
    ...
    router.post('/api/bridge/led', async (req, res) => {
        try {
            ...
            const led = await engine.setLed({ on, level });
    ...
    // Сырой статус устройства Tuya (диагностика DP-кодов)
...
function initBridge({ app, io } = {}) {
    ...
    engine.startScheduler();
    engine.log('info', 'Мост Tuya инициализирован');

    return { config, tuya, engine };
}

module.exports = { initBridge, config, tuya, engine };
